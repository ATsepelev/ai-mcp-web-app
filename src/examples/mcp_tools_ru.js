...
/**
 * Регистрирует обработчики инструментов на переданном сервере.
 * @param {MCPServer} mcpServer Экземпляр MCPServer.
 */
export function registerTools(mcpServer) {
  if (!mcpServer || typeof mcpServer.onRequest !== 'function') {
    console.error('[MCP Tools] Invalid server instance provided.');
    return;
  }

  // Вспомогательная функция для установки значения в React-элемент
  ...
  console.log('[MCP Tools] Handlers registered.');
}